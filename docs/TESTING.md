...
Dev environment. Follow the scenarios below to verify features. Index: [docs.md](docs.md). Engineering agreement: [ia.md](ia.md). See [ai.md](ai.md), [reference.md](reference.md).
...
- **Templates:** Fill form → Save as template (toolbar) → name. Open Templates → select → form pre-filled → Create.
...
- **Reminders:** Overdue + due ≤24h → Reminders panel; complete overdue → disappears.
- **Analytics:** Open Analytics → totals, active, completed, overdue, high priority, weekly/monthly, priority breakdown; updates with data.

---

### Test 9: Time tracking, history, estimates

- **Timer:** Start on task → increments; Stop → time saved. Start on another → previous stops. Persists after refresh.
- **History:** Task history control on task → modal with actions (newest first); create/edit/complete/archive → logged.
...
- **Theme:** Toggle dark/light → persists. Settings → accent color → applies and persists.
- **Settings:** Long page; scroll down to Filter Presets → Back to top control appears → click to scroll to top. **Tasks:** same Back to top when scrolled.
...
- Defaults: priority, date format, sort → Save → confirmation shown → new task uses defaults; persist after refresh.
...
+ Add Task → AI menu → text input: e.g. `Review report by Friday, high priority` → Add from text. Modal opens with title, due (Friday), priority High; edit if needed → Create. **Tags** from parse are normalized to first letter capital (e.g. Report, Work). Empty input → button disabled. With LLM key: API parse; without: keyword fallback (e.g. "high priority", "by Friday"). Quick checks: `tomorrow morning jog high prio`, `Call mom tomorrow`, `low prio admin task next week`, `meeting at 10am` (no date) — see [ai.md](ai.md).
